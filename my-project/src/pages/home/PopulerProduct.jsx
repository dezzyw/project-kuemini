      ...
      <div className="max-w-7xl mx-auto px-8 md:px-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 justify-center">
          {products.map((product, index) => (
            <div
              key={index}
              className="bg-[#FE8A8A] rounded-2xl shadow-lg p-6 max-w-[360px] h-auto mx-auto text-white flex flex-col items-center transform transition-transform duration-300 hover:scale-105 hover:shadow-xl"
            >
              <img
                src={product.img}
                alt={product.title}
              ...
              <button className="mt-4 px-6 py-2 border-2 border-white rounded-full text-white hover:bg-white hover:text-[#FE8A8A] transition">
                Selengkapnya
              </button>
            </div>
          ))}
